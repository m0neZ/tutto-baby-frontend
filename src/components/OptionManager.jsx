...
const OptionManager = ({ type }) => {
  ...
  return (
    // Removed mt-6 as parent container (AdminPage) now provides padding
    <div>
      <h2 className="text-xl font-semibold text-[--color-primary] mb-4">{labels[type]}</h2>
      ...
      <div className="flex gap-3 mb-5">
        <input
          ...
          className="flex-grow px-3 py-2 border border-[--color-accent]/70 rounded-md focus:ring-1 focus:ring-[--color-primary] focus:outline-none focus:border-[--color-primary]/50 transition-shadow duration-150 shadow-sm"
        />
        <button
          onClick={addOption}
          className="flex items-center bg-[--color-primary] text-white px-4 py-2 rounded-md shadow-sm hover:bg-[--color-secondary] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[--color-primary]/50"
        >
      ...
      {loading && <p className="text-[--color-text-secondary] mb-4 text-sm">Carregando...</p>}
      ...
      <div className="space-y-2">
        {activeOptions.map(opt => (
          <div
            key={opt.id}
            className="bg-[--color-background] p-3 rounded-md border border-[--color-accent]/50 flex justify-between items-center shadow-sm"
          >
            <span className="text-[--color-text-primary] text-sm font-medium">{opt.value}</span>
            <button
      ...
      {inactiveOptions.length > 0 && (
        <div className="mt-6 pt-4 border-t border-[--color-accent]/50">
          <h4 className="text-base font-medium text-[--color-text-secondary] mb-3">Inativos</h4>
          <div className="space-y-2">
      ...
      {!loading && options.length === 0 && (
          <p className="text-[--color-text-secondary] mt-4 text-sm text-center py-5">Nenhuma opção cadastrada para {labels[type].toLowerCase()}.</p>
      )}
    </div>
  );
};
